...
async function carregarProcessos() {
  try {
    const [processos, clientes, veiculos] = await Promise.all([
    ...
    return `
      <tr>
        ...
        <td>
          <div class="acoes">
            ${p.status !== 'concluido' ? `
              <button class="btn-icone concluir" title="Concluir" onclick="concluirProcesso(${p.id})">&#10003;</button>
            ` : ''}
...
async function abrirChecklist(processoId, tipo) {
  ...
  try {
    let itens = await fetch(`${API}/checklist/${processoId}`).then(r => r.json());
...
function renderizarChecklist(itens) {
  ...
  document.getElementById('checklistItens').innerHTML = `
    <div class="checklist-progresso">
      Documentos entregues: ${entregues} de ${total}
    </div>
    ${itens.map(item => `
