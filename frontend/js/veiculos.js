...
function renderizarTabela(veiculos) {
  ...
  tbody.innerHTML = veiculos.map(v => `
    <tr>
      ...
      <td>
        <div class="acoes">
          <button class="btn-icone detran" title="Consultar IPVA/Licenciamento (SEF-MG)" onclick="abrirSEF('${v.renavam}')">SEF</button>
          <button class="btn-icone detran2" title="Consultar situação no Detran MG" onclick="abrirDetran()">DT</button>
...
function abrirSEF(renavam) {
  if (!renavam) {
    alert('Este veículo não possui RENAVAM cadastrado.');
    return;
  }
  window.open('https://veiculosmg.fazenda.mg.gov.br/buscar-renavam/', '_blank');
}

function abrirDetran() {
  window.open('https://transito.mg.gov.br/veiculos/face-do-veiculo/consultar-situacao-do-veiculo', '_blank');
}
